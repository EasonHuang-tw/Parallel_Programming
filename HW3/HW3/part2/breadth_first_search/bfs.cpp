    ...
    int iteration)
{
    ...
    #pragma omp parallel 
    {
        #pragma omp for reduction(+ : local_count) schedule(guided)
        for (int i=0; i < g->num_nodes; i++) {   
            if (frontier->vertices[i] == iteration) {             
                ...
                for (int neighbor=start_edge; neighbor<end_edge; neighbor++) {
                    ...
                    // if(__sync_bool_compare_and_swap(&frontier->vertices[outgoing], BOTTOMUP_NOT_VISITED_MARKER, iteration + 1)) {                  
...
void bfs_hybrid(Graph graph, solution* sol) {
    ...
    while (frontier->count != 0) {
        ...
        if(frontier->count >= THRESHOLD) {
            ...
            frontier->count = 0;
            h_top_down_step(graph, frontier, sol->distances, iteration);
/*
